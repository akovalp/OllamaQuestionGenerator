...
# --- Constants ---
MAX_ITERATIONS = 6
NUM_QUESTIONS = 5  # Default number of questions (can be overridden by frontend)
DEFAULT_MODEL = "gemma:2b" # Changed Default model to smaller one for faster generation
...
# --- Caching Decorator ---
# Note: This is a general cache, not specifically tied to Streamlit's caching mechanism
def simple_cache(ttl: int = 3600):
    """Custom cache decorator with time-to-live."""
    cache = {}
    def decorator(func):
        @wraps(func)
...
def get_available_models() -> List[str]:
    ...
    try:
        ...
        if not available_models:
             available_models = [DEFAULT_MODEL] # Fallback if list is empty but ollama responded
        return available_models
    except Exception as e:
        print(f"Warning: Could not fetch models from Ollama: {e}. Using default.")
...
class TextGenerator:
    ...
    def __init__(self, model: str = DEFAULT_MODEL):
        ...
        if model not in AVAILABLE_MODELS:
             print(f"Warning: Specified model '{model}' not found in Ollama list. Falling back to '{self.model}'.")
    ...
    # Apply the cache decorator to the generation methods if desired
    # @simple_cache(ttl=3600) # Example usage
    def generate_text(self, topic: str, language: str, level: str, style: str) -> Dict[str, Any]:
        ...
        print(f"Generating text with model: {self.model}") # Log which model is used
    ...
    def _generate_text_english(self, topic: str, language: str, level: str, style: str) -> Dict[str, Any]:
        ...
        for _ in range(MAX_ITERATIONS):
            iterations += 1
            prompt = self._create_english_prompt(topic, level, language, style, best_score)

            try:
                response = ollama.generate(
                    model=self.model,
                    prompt=prompt,
                    system="You are a professional language teacher tasked to create a reading passage. Do not give any output besides the text."
                )
                ...
                try:
                    ...
                    print("Warning: textstat.gunning_fog failed. Using fallback difficulty estimation.")
                ...
                elif best_difference is None or score_difference < best_difference:
                    best_text = generated_text
                    best_score = score
                    best_difference = score_difference

            except Exception as e:
                print(f"Error during text generation iteration {iterations}: {e}")
                if best_text: # If we got something usable before the error, use it
                    break
                     ...
                     raise Exception(f"Failed to generate text using model {self.model}: {e}")


        if best_text is None:
             # Handle case where loop finishes without successful generation (e.g., Ollama errors)
             raise Exception(f"Could not generate suitable text after {iterations} iterations using model {self.model}.")
    ...
    def _generate_text_other_languages(self, topic: str, language: str, level: str, style: str) -> Dict[str, Any]:
        ...
        prompt = self._create_other_language_prompt(topic, level, language, style)
        try:
            response = ollama.generate(
                model=self.model,
                prompt=prompt,
            ...
            return result.model_dump()
             ...
             raise Exception(f"Error generating non-English text with model {self.model}: {e}")
    ...
    def _create_english_prompt(self, topic: str, level: str, language: str, style: str,
        ...
        words_range = {"Basic": "150-250", "Intermediate": "250-400", "Advanced": "400-600"}
        prompt = f"""
        ...
        if previous_score is not None:
            ...
            else: # Score was outside range but not strictly above/below (edge case?)
    ...
    def _estimate_difficulty(self, text: str) -> float:
        ...
        try:
             ...
             complex_words = sum(1 for word in words if len(word) > 6) # Adjust threshold as needed
             percent_complex = (complex_words / num_words) * 100 if num_words else 0
...
class QuestionGenerator:
    ...
    # @simple_cache(ttl=3600) # Example usage
    def generate_questions(self, generated_text: str, num_questions: int, language: str) -> Dict[str, Any]:
        ...
        2.  Provide exactly three plausible answer choices (options). One choice must be the correct answer based on the text.
        ...
        Each question object should have the keys "question" (string), "choices" (list of 3 strings), and "answer" (string - the correct choice text).
        ...
        print(f"Generating questions with model: {self.model}") # Log model used

        try:
            response = ollama.chat(
                model=self.model,
                messages=[
                    {"role": "system", "content": f"You are an AI assistant specialized in creating multiple-choice comprehension questions based on provided text. Respond ONLY with the requested JSON object containing the questions. Ensure all text content (questions, choices, answers) is in {language}."},
                    {"role": "user", "content": prompt}
                ],
                format='json', # Request JSON format directly
                options={"temperature": 0.5} # Lower temperature for more predictable JSON structure
            )
            ...
            try:
                ...
                validated_data = Questions.model_validate_json(json_response_str)
                # Ensure the correct number of questions were generated
                if len(validated_data.questions) != num_questions:
                     print(f"Warning: Requested {num_questions} questions, but model generated {len(validated_data.questions)}. Using generated questions.")
                ...
                return validated_data.model_dump() # Return Pydantic model output as dict
                 ...
                 print(f"Error: Failed to parse or validate JSON response from model {self.model}.")
                 print(f"Model Response String: {json_response_str}")
                 raise Exception(f"Could not parse valid JSON questions from the model: {json_error}")


        except Exception as e:
             # Catch errors from ollama.chat or JSON parsing
             raise Exception(f"Error generating questions with model {self.model}: {e}")
        ...
        if language == "English":
            try:
                ...
                if word_count > 10: # Basic check if text is long enough for stats
                     metrics["difficulty_gunning_fog"] = round(textstat.gunning_fog(text), 2)
                     metrics["sentence_count"] = textstat.sentence_count(text)
                     # Calculate reading time in minutes
                     reading_time_seconds = textstat.reading_time(text, ms_per_char=14.6) # Standard ~200 WPM
                     metrics["reading_time_min"] = round(reading_time_seconds / 60, 1)
                else:
                     metrics["difficulty_gunning_fog"] = "N/A (Too short)"
                     metrics["sentence_count"] = text.count('.') + text.count('!') + text.count('?') # Basic count
                     metrics["reading_time_min"] = "~0.1" # Estimate for short text
            except Exception as e:
                print(f"Warning: textstat calculation failed for English text: {e}. Metrics might be incomplete.")
                ...
                if "reading_time_min" not in metrics:
                     ...
                     metrics["reading_time_min"] = round(word_count / 200, 1) if word_count > 0 else 0.0
                if "difficulty_gunning_fog" not in metrics:
                     ...
                     metrics["difficulty_gunning_fog"] = round(temp_gen._estimate_difficulty(text), 2) if word_count > 10 else "N/A (Too short)"
                     metrics["difficulty_gunning_fog"] = f"{metrics['difficulty_gunning_fog']} (est.)"


        else: # For non-English languages
             metrics["sentence_count"] = text.count('.') + text.count('!') + text.count('?')
             metrics["reading_time_min"] = round(word_count / 180, 1) if word_count > 0 else 0.0 # Slightly slower WPM estimate
